...
const Questions = () => {
    ...
    useEffect(() => {
        const generateQuiz = async () => {
            setIsLoading(true)
            try {
                const prompt = `Create a multiple-choice quiz on the ${topic} technology. Provide exactly 4 questions, each with 4 answer options and a correct answer. Return the result in a JSON array format where each object has this structure: { "question": "string", "options": ["string", "string", "string", "string"], "correctAnswer": "string" }. Do not include markdown formatting like \`\`\`json.`;
                ...
                if (!content) {
                    ...
                    if (
                        ...
                        setError(true)
                        Alert.alert('Error', 'Invalid data format from Gemini')
                    }
                }
            } catch (error: any) {
                setError(true)
            ...
            </Text>
            {
                questions[currentQuestionIndex]?.options.map((option, index) => (
                    <Pressable
                        ...
                        className={`border-purple-500 border p-4 my-2 rounded-md ${selectedOption === option
                            ? isCorrect
